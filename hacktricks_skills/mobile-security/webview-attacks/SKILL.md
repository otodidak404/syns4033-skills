---
name: hacktricks-webview-attacks
description: >-
  Hacktricks playbook: Webview Attacks. Exploitation techniques, enumeration, and attack patterns.
...
- **File Access From File URLs**: This feature, also deprecated, controlled access to content from other file scheme URLs. Like universal access, its default is disabled for enhanced security.
  - Use `getAllowFileAccessFromFileURLs()` to check and `setAllowFileAccessFromFileURLs(boolean)` to set.<sup>[[1]](#references)</sup>
...
The **WebViewAssetLoader** class is the modern approach for loading local files. It uses http(s) URLs for accessing local assets and resources, aligning with the Same-Origin policy, thus facilitating CORS management.<sup>[[1]](#references)</sup>
...
Many apps register custom schemes/paths that route a user-supplied URL into an in-app WebView. If the deep link is exported (VIEW + BROWSABLE), an attacker can force the app to render arbitrary remote content inside its WebView context.<sup>[[4]](#references)</sup>
...
A frequent hardening mistake is enabling JavaScript or configuring relaxed WebView settings before the final allowlist/verification of the target URL completes. If the verification is inconsistent across helpers or happens too late, an attacker deep link can reach a state where:<sup>[[6]](#references)[[7]](#references)[[8]](#references)</sup>
...
Why it’s exploitable<sup>[[6]](#references)[[7]](#references)</sup>
...
- **Extreme caution is required** for apps targeting Android versions below 4.2 due to a vulnerability allowing remote code execution through malicious JavaScript, exploiting reflection.<sup>[[3]](#references)</sup>
...
A common pattern is a single exported method (e.g., `@JavascriptInterface void invokeMethod(String json)`) that deserializes attacker-controlled JSON into a generic object and dispatches based on a provided handler name. Typical JSON shape:<sup>[[10]](#references)</sup>
...
Risk: if any registered handler performs privileged actions on attacker data (e.g., direct file reads), you can call it by setting `handlerName` accordingly. Results are usually posted back into the page context via `evaluateJavascript` and a callback/promise mechanism keyed by `callbackId`.<sup>[[10]](#references)</sup>

Key hunting steps<sup>[[10]](#references)</sup>
...
Detection tips<sup>[[10]](#references)</sup>
...
This is not an origin check. Many unintended hosts satisfy the second clause, letting untrusted domains into the privileged Activity. Always verify scheme and host against a strict allowlist (exact match or a correct subdomain check with dot-boundaries), not `endsWith` tricks.<sup>[[10]](#references)</sup>

#### javascript:// execution primitive via loadUrl

Once inside a privileged WebView, apps sometimes execute inline JS via:<sup>[[10]](#references)</sup>

```java
webView.loadUrl("javascript:" + jsPayload);
```

If an internal flow triggers `loadUrl("javascript:...")` in that context, injected JS executes with bridge access even if the external page wouldn’t normally be allowed. Pentest steps:<sup>[[10]](#references)</sup>
...
Mitigations (developer checklist)<sup>[[10]](#references)</sup>
...
- In DevTools, type the bridge object name (e.g., `xbridge`) to see exposed members and probe the dispatcher.<sup>[[10]](#references)</sup>
...
- Demonstrates the exfiltration of arbitrary files using an XMLHttpRequest:<sup>[[2]](#references)</sup>
...
A frequent vulnerability is reading attacker-controlled data from an incoming `Intent` extra and injecting it directly into a WebView via `loadData()` with JavaScript enabled.<sup>[[9]](#references)</sup>
...
Hunting tips:<sup>[[13]](#references)</sup>
...
Many bridges return data to the page through a global callback such as `window.callWebView(...)`. If a bridge action returns a **JWT/session token** (`refresh_jwt`, `getToken`, `getSession`, etc.), preserve normal app behaviour by wrapping the callback, extracting the sensitive field, and then calling the original handler:<sup>[[13]](#references)</sup>
...
Practical notes:<sup>[[13]](#references)</sup>
...
- [1] [Review of Android WebViews file access attack vectors](https://labs.integrity.pt/articles/review-android-webviews-fileaccess-attack-vectors/index.html)
- [2] [WheresMyBrowser.Android (demo app)](https://github.com/authenticationfailure/WheresMyBrowser.Android)
- [3] [Android WebView reference](https://developer.android.com/reference/android/webkit/WebView)
- [4] [Deep Links & WebViews Exploitations – Part II](https://medium.com/@justmobilesec/deep-links-webviews-exploitations-part-ii-5c0b118ec6f1)
- [5] [Deep Links & WebViews Exploitations – Part I](https://www.justmobilesec.com/en/blog/deep-links-webviews-exploitations-part-I)
- [6] [Samsung S24 Exploit Chain Pwn2Own 2024 Walkthrough](https://medium.com/@happyjester80/samsung-s24-exploit-chain-pwn2own-2024-walkthrough-c7a3da9a7a26)
- [7] [Pwn2Own Ireland 2024 – Samsung S24 attack chain (whitepaper)](https://maliciouserection.com/2025/05/13/pwn2own-ireland-2024-samsung-s24-attack-chain-whitepaper.html)
- [8] [Demonstration video](https://www.youtube.com/watch?v=LAIr2laU-So)
- [9] [Android Intents (1/2): how they work, security, and attack examples – Mobeta](https://mobeta.fr/android-intent-hijacking-pentest-mobile/)
- [10] [Account takeover in Android app via JSB – tuxplorer.com](https://tuxplorer.com/posts/account-takeover-via-jsb/)
- [11] [LSPosed – systemless Xposed framework](https://github.com/LSPosed/LSPosed)
- [12] [Frida codeshare: Cordova – enable WebView debugging](http://codeshare.frida.re/@gameFace22/cordova---enable-webview-debugging/)
