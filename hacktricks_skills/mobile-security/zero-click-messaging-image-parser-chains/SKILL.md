---
name: hacktricks-zero-click-messaging-image-parser-chains
description: >-
  Hacktricks playbook: Zero-click Messaging → Image Parser Chains. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: Zero-click Messaging → Image Parser Chains
...
The WhatsApp "linked devices" architecture keeps the primary phone and every companion (desktop, tablet, secondary phone) in sync via encrypted, structured protocol messages. Each message encodes:<sup>[[1]](#references)</sup>
...
On Apple clients, the handler that processes these linked-device control packets **implicitly trusted** that a valid pairing already occurred, so high-impact fields (e.g., `resource_url`, `open_media`, `sync_snapshot`) were only minimally validated. A malicious companion message could therefore:<sup>[[1]](#references)</sup>
...
### Don't stop at linked-device sync
...
print('SOF3 NumComponents =', data[sof3 + 9])
print('DHT markers =', data.count(b'\xff\xc4'))
PY
```

Quarkslab's patch diff adds a useful constraint: RawCamera only reaches the buggy decompressor when the file also carries a `SOF3` lossless-JPEG marker and enough `DHT` tables for the declared `NumComponents`. A practical trigger recipe is therefore **`SamplesPerPixel == 2` in TIFF metadata, `SOF3 NumComponents == 1`, and a valid single-component Huffman setup**.<sup>[[3]](#references)</sup>
...
Each crash in `RawCamera` gives you a new primitive. The published PoC achieved a neat out-of-bounds read/write reliable enough to crash WhatsApp on iPhone, iPad, and Mac.<sup>[[1]](#references)</sup>
...
Because every step is automatic, the attacker only needs the victim’s phone number. No notifications, banners, or prompts are shown on the target device.<sup>[[1]](#references)</sup>
...
- **BLASTPASS (September 7, 2023 / iOS 16.6.1)**: Citizen Lab reported malicious **PassKit** attachments containing images, and Apple patched `Wallet` (`CVE-2023-41061`) plus `ImageIO` (`CVE-2023-41064`). Project Zero later refined the usual "BlastDoor bypass" summary: many observed sample crashes still happened **inside** BlastDoor, likely because PKPass parsing accepted mislabeled embedded assets, but later crash logs also showed a `CKPassPreviewMediaObject` path that reached WebP in `MobileSMS` outside BlastDoor. The reusable lesson is not "PassKit always escapes the sandbox", but **trace which preview object and which process actually decodes the embedded file** for every wrapper (`.pkpass`, contact cards, sticker bundles, inline HTML, QuickLook previews).<sup>[[2]](#references)</sup>
- **TRIANGULATION (patched in iOS 15.7.8 on July 24, 2023, with the mainline fix already in iOS 16.3)**: Kaspersky showed that a malicious iMessage attachment hit the undocumented Apple-only `ADJUST` TrueType instruction (`CVE-2023-41990`). The practical lesson is that **fonts are image-parser cousins** for zero-click work: rich-text previews, font fallback, and thumbnail generation can all become parser entry points even when the app claims to only support "documents" or "stickers".<sup>[[5]](#references)</sup>
...
Samsung’s bulletin for CVE-2025-21043 confirmed an **out-of-bounds write** in `libimagecodec.quram.so`, a proprietary image parser reported by the Meta and WhatsApp security teams.<sup>[[6]](#references)</sup> The exploitation methodology mirrors the Apple chain:<sup>[[1]](#references)</sup>

- Identify an auto-preview vector (chat thumbnails, notification previews, share sheets, gallery rescans) that parses the attacker file with Samsung’s vendor image stack.
...
Once an OOB write exists in the vendor parser, combining it with the WhatsApp auto-fetch primitive yields another zero-click chain on Samsung devices.<sup>[[1]](#references)</sup>
...
- [1] [DNGerousLINK: A Deep Dive into WhatsApp 0-Click Exploits on iOS and Samsung Devices](https://media.ccc.de/v/39c3-dngerouslink-a-deep-dive-into-whatsapp-0-click-exploits-on-ios-and-samsung-devices)
- [2] [Project Zero: Blasting Past WebP](https://projectzero.google/2025/03/blasting-past-webp.html)
- [3] [Quarkslab: Reverse engineering of Apple's iOS 0-click CVE-2025-43300](https://blog.quarkslab.com/patch-analysis-of-Apple-iOS-CVE-2025-43300.html)
- [4] [WhatsApp Security Advisories 2025](https://www.whatsapp.com/security/advisories/2025)
- [5] [Kaspersky Securelist: Operation Triangulation: The last (hardware) mystery](https://securelist.com/operation-triangulation-the-last-hardware-mystery/111669/)
- [6] [Samsung Mobile Security Bulletin – SMR Sep-2025 Release 1 (SVE-2025-21043 / CVE-2025-21043)](https://security.samsungmobile.com/securityUpdate.smsb?year=2025&month=9)
