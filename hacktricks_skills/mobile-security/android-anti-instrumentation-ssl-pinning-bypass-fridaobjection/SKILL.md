---
name: hacktricks-android-anti-instrumentation-ssl-pinning-bypass-fridaobjection
description: >-
  Hacktricks playbook: Android Anti-Instrumentation & SSL Pinning Bypass (Frida/Objection). Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: Android Anti-Instrumentation & SSL Pinning Bypass (Frida/Objection)
...
Newer banking/ID apps tie runtime checks to Google Play Integrity (SafetyNet replacement) and can also crash if Zygisk itself is present.<sup>[[15]](#references)</sup> Quick triage tips:
...
Medusa provides 90+ ready-made modules for SSL unpinning, root/emulator detection bypass, HTTP comms logging, crypto key interception, and more.<sup>[[10]](#references)</sup>
...
Auto-Frida is a Frida automation toolkit that focuses on repeatable setup plus **auto-detection** of protections and **consolidated bypass script generation**. It is useful when apps run checks very early or when multiple bypass modules would otherwise double-hook the same APIs.<sup>[[11]](#references)</sup>
...
Override custom guards to return safe values without repacking:<sup>[[1]](#references)</sup>
...
<details>
<summary>Frida example: dump client cert/private key from a decrypted PKCS12-backed KeyStore</summary>
...
Trace JNI entry points to locate native loaders and detection init:<sup>[[1]](#references)</sup>
...
If instrumentation is blocked, you can still inspect traffic by removing pinning statically:<sup>[[1]](#references)</sup>
...
On rooted devices, LSPosed/Xposed modules can hook Java telephony/SMS APIs at runtime, keeping the APK unmodified on disk while fully controlling what the app sees. This is commonly abused to bypass SIM‑binding flows that trust local telephony APIs or local SMS provider state.<sup>[[16]](#references)</sup>
...
- [1] [Reversing Android Apps: Bypassing Detection Like a Pro](https://www.kayssel.com/newsletter/issue-12/)
- [2] [Frida Codeshare](https://codeshare.frida.re/)
- [3] [Objection](https://github.com/sensepost/objection)
- [4] [apk-mitm](https://github.com/shroudedcode/apk-mitm)
- [5] [Jadx](https://github.com/skylot/jadx)
- [6] [Ghidra](https://ghidra-sre.org/)
- [7] [r2frida](https://github.com/nowsecure/r2frida)
- [8] [Apktool install guide](https://apktool.org/docs/install)
- [9] [Magisk](https://github.com/topjohnwu/Magisk)
- [10] [Medusa (Android Frida framework)](https://github.com/Ch0pin/medusa)
- [11] [Auto-Frida (Android Frida automation toolkit)](https://github.com/ommirkute/Auto-Frida)
- [12] [Build a Repeatable Android Bug Bounty Lab: Emulator vs Magisk, Burp, Frida, and Medusa](https://www.yeswehack.com/learn-bug-bounty/android-lab-mobile-hacking-tools)
- [13] [phantom-frida (stealth Frida server builder)](https://github.com/TheQmaks/phantom-frida)
- [14] [Frida OkHttp4 SSL pinning bypass script](https://github.com/Zero3141/Frida-OkHttp-Bypass)
- [15] [XDA guide to strong Play Integrity bypass (2025)](https://xdaforums.com/t/updated-11-17-2025-guide-get-strong-integrity-fix-banking-apps-revolut-google-wallet-android-16-working.4753805/)
