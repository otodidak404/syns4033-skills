---
name: hacktricks-install-burp-certificate
description: >-
  Hacktricks playbook: Install Burp Certificate. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: Install Burp Certificate
...
Configure a global HTTP proxy so all apps route traffic through your interceptor (Burp/mitmproxy):<sup>[[6]](#references)</sup>
...
![Clear proxy - On a Virtual Machine: First of all you need to download the Der certificate from Burp. You can do this in Proxy -- Options -- Import / Export CA certificate](<../../images/image (367).png>)
...
Explained in [**this video**](https://www.youtube.com/watch?v=qQicUW0svB8) you need to:<sup>[[7]](#references)</sup>
...
### Rooted Play Store AVD workflow (no Frida required)
...
1. Root the matching Play Store image (`google_apis_playstore`) with **rootAVD**, cold boot it, finish the **Magisk** setup, and verify root from ADB:<sup>[[2]](#references)</sup>
...
2. Prepare the proxy CA in Android's **system CA** naming format (`<subject_hash_old>.0`). Burp exports DER directly; Caido usually exports PEM first:

```bash
# Burp DER -> Android CA filename
openssl x509 -inform DER -subject_hash_old -in cacert.der | head -1
mv cacert.der <hash>.0

# Caido PEM -> DER -> Android CA filename
openssl x509 -in ca.crt -outform DER -out caido.der
openssl x509 -inform DER -subject_hash_old -in caido.der | head -1
mv caido.der <hash>.0

adb push <hash>.0 /storage/self/primary/
```

3. Run the **Post Android 14** bind-mount technique below as `root` so `/system/etc/security/cacerts` is overlaid with `tmpfs`, the original certs are restored, the new CA is added, and the modified directory is bind-mounted into `/apex/com.android.conscrypt/cacerts` for **Zygote/Zygote64** and already running apps.

4. This change is **runtime only** on the emulator: after every reboot, rerun the CA-injection script **before** testing. Then point the device at Burp/Caido with `adb shell settings put global http_proxy <ip>:<port>` and clear it later with `adb shell settings put global http_proxy :0`.

This bypasses trust decisions that rely only on the Android system CA store, but it is **not** a universal unpinning technique: apps with hardcoded/public-key/native pinning can still require [Frida hooks or patching](android-anti-instrumentation-and-ssl-pinning-bypass.md).
...
Note: Some Android 12/13 devices that received **Conscrypt Mainline** updates already use `/apex/com.android.conscrypt/cacerts`. If that directory exists on your device, you must use the same APEX injection technique described below.<sup>[[4]](#references)</sup>
...
Tip: If `/system/etc/security/cacerts` contains nested mounts (common with Magisk modules), use `--rbind` instead of `--bind` so those mounts propagate into app namespaces.<sup>[[4]](#references)</sup>
...
- [1] [Bypassing SSL Pinning on Play Store AVDs without Frida](https://www.mfumis.com/posts/bypassing-ssl-pinning-on-play-store-avds-without-frida/)
- [2] [rootAVD](https://gitlab.com/newbit/rootAVD)
- [3] [Android 14: Install a system CA certificate on a rooted device](https://httptoolkit.com/blog/android-14-install-system-ca-certificate/)
- [4] [Intercepting traffic on Android with Mainline and Conscrypt](https://blog.nviso.eu/2025/06/05/intercepting-traffic-on-android-with-mainline-and-conscrypt/)
- [5] [AlwaysTrustUserCerts Magisk module](https://github.com/NVISOsecurity/AlwaysTrustUserCerts)
- [6] [Build a Repeatable Android Bug Bounty Lab: Emulator vs Magisk, Burp, Frida, and Medusa](https://www.yeswehack.com/learn-bug-bounty/android-lab-mobile-hacking-tools)
- [7] [Root Android Studio Device with Magisk and Install Burp Certificate as System cert in (4 Easy Steps)](https://www.youtube.com/watch?v=qQicUW0svB8)
