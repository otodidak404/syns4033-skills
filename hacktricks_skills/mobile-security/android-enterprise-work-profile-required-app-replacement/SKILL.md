---
name: hacktricks-android-enterprise-work-profile-required-app-replacement
description: >-
  Hacktricks playbook: Android Enterprise Work Profile Required-App Replacement. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: Android Enterprise Work Profile Required-App Replacement
...
Even after the **CVE-2023-21257** patch that blocks ADB sideloads when `DISALLOW_INSTALL_APPS` or `DISALLOW_DEBUGGING_FEATURES` are set, the following chain lets an attacker **replace any Intune-required Work Profile app** with arbitrary code:<sup>[[1]](#references)</sup>
...
* Confirm multi-user layout and user IDs:<sup>[[1]](#references)</sup>
...
Android Studio's Run/Debug configuration can still push builds with the **`INSTALL_ALL_USERS`** flag. Before running, enable *Deploy as instant app* → *Install for all users*.<sup>[[1]](#references)</sup>

Build the malicious payload with the **same package name** as the managed app and a **much larger `versionCode`** so PackageManager/Finsky treats it as a newer release:<sup>[[1]](#references)</sup>
...
When Android Studio deploys:<sup>[[1]](#references)</sup>
...
Within ~1–10 minutes (policy refresh interval):<sup>[[1]](#references)</sup>
...
The malicious binary now resides inside the Work Profile under the genuine package name and is considered compliant by the MDM.<sup>[[1]](#references)</sup>

## Post-exploitation opportunities

* **Work-profile data access** – other enterprise apps keep trusting Intents/content providers bound to the replaced package, enabling internal data theft and covert exfiltration from the Work Profile to attacker infrastructure.<sup>[[1]](#references)</sup>
...
- [1] [Bypassing CVE-2023-21257 via Intune Required-App Auto-Install](https://jgnr.ch/sites/android_enterprise.html)
