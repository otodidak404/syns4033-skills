---
name: hacktricks-react-native-application-analysis
description: >-
  Hacktricks playbook: React Native Application Analysis. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: React Native Application Analysis
...
As the bundle contains actually all the JS code of the application it's possible to **divide it in different files** (potentially making easier its reverse engineering) using the **tool [react-native-decompiler](https://github.com/numandev1/react-native-decompiler)**.<sup>[[3]](#references)</sup>
...
To search for sensitive credentials and endpoints, follow these steps:<sup>[[1]](#references)</sup>
...
These simple greps often surface interesting indicators even in minified JS:<sup>[[2]](#references)</sup>
...
Some apps accidentally ship classes that make Dev Support togglable. If present, you can try forcing `getUseDeveloperSupport()` to return true:<sup>[[7]](#references)</sup>
...
- react-native-mmkv (Android): versions prior to 2.11.0 logged the optional encryption key to Android logs. If ADB/logcat is available, secrets could be recovered. Ensure >= 2.11.0. Indicators: usage of `react-native-mmkv`, log statements mentioning MMKV init with encryption. CVE-2024-21668.<sup>[[4]](#references)</sup>
...
- [1] [Let's know how I have explored the buried secrets in React Native application](https://medium.com/bugbountywriteup/lets-know-how-i-have-explored-the-buried-secrets-in-react-native-application-6236728198f7)
- [2] [Expanding the attack surface: React Native Android applications](https://www.assetnote.io/resources/research/expanding-the-attack-surface-react-native-android-applications)
- [3] [Mastering React Native Application Pentesting - A Practical Guide](https://payatu.com/wp-content/uploads/2023/02/Mastering-React-Native-Application-Pentesting-A-Practical-Guide-2.pdf)
- [4] [CVE-2024-21668 - react-native-mmkv logs encryption key on Android (NVD)](https://nvd.nist.gov/vuln/detail/CVE-2024-21668)
- [5] [hbctool (and forks) for Hermes assemble/disassemble](https://github.com/bongtrop/hbctool)
- [6] [Äike BLE authentication bypass: default BLE private key allows unlocking any nearby scooter](https://blog.nns.ee/2026/01/06/aike-ble/)
- [7] [Hooking React Native Applications with Frida](https://newsroom.bedefended.com/hooking-react-native-applications-with-frida/)
