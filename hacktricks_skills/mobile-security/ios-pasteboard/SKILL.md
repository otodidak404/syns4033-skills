---
name: hacktricks-ios-pasteboard
description: >-
  Hacktricks playbook: iOS Pasteboard. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: iOS Pasteboard
...
Data sharing within and across applications on iOS devices is facilitated by `UIPasteboard`, which has two primary categories:<sup>[[1]](#references)</sup><sup>[[4]](#references)</sup>
...
**Security considerations** play a significant role when utilizing pasteboards. For instance:<sup>[[1]](#references)</sup><sup>[[2]](#references)</sup>

- Pasteboard access is not governed by a traditional per-app permission toggle. Modern iOS versions surface paste-access notifications and may prompt for permission when an app directly reads content from another app; system paste controls can provide user-initiated paste without that prompt. Treat behavior as OS-version dependent.<sup>[[4]](#references)</sup><sup>[[5]](#references)</sup>
...
For static analysis, search the source code or binary for:<sup>[[3]](#references)</sup>
...
Dynamic analysis involves hooking or tracing specific methods:<sup>[[3]](#references)</sup>
