---
name: hacktricks-android-in-memory-native-code-execution-via-jni-shellcode
description: >-
  Hacktricks playbook: Android In-Memory Native Code Execution via JNI (shellcode). Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: Android In-Memory Native Code Execution via JNI (shellcode)
...
Declare `<uses-permission android:name="android.permission.INTERNET" />`, run the network operation off the UI thread, close the stream, enforce a maximum response size, and authenticate the payload. `InputStream.readAllBytes()` is not available on every Android API level, so use a bounded compatibility helper or an HTTP client when targeting older devices.<sup>[[6]](#references)</sup>
...
Notes and caveats
- W^X/execmem: the sample never maps a page writable and executable at the same time; it transitions RW to RX. Whether an anonymous executable mapping is permitted depends on Android version, app domain, SELinux/vendor policy, and device hardening. ART does maintain a managed JIT code cache, preserving the earlier page's useful executable-pool distinction, but that cache is not a general JNI API or a drop-in policy bypass. Treat `mprotect()` failure as a stop condition rather than attempting to evade policy.<sup>[[9]](#references)</sup>
...
A robust operator pipeline is to:<sup>[[2]](#references)[[3]](#references)</sup>
...
- [1] [CoRPhone challenge repo (Android kernel pwn; JNI memory-only loader pattern)](https://github.com/0xdevil/corphone)
- [2] [build.sh (musl-gcc + pwntools pipeline)](https://raw.githubusercontent.com/0xdevil/corphone/main/exploit/build.sh)
- [3] [exp2sc.py (pwntools shellcraft.loader_append)](https://raw.githubusercontent.com/0xdevil/corphone/main/exploit/exp2sc.py)
- [4] [exploit.c TL;DR (operator/kernel flow, offsets, reverse shell)](https://raw.githubusercontent.com/0xdevil/corphone/main/exploit/exploit.c)
- [5] [INSTRUCTIONS.md (setup notes)](https://github.com/0xdevil/corphone/blob/main/INSTRUCTIONS.md)
- [6] [Android Developers - Connect to the network](https://developer.android.com/develop/connectivity/network-ops/connecting)
- [7] [pwntools documentation - AMD64 Linux `loader_append`](https://docs.pwntools.com/en/stable/shellcraft/amd64.html#pwnlib.shellcraft.amd64.linux.loader_append)
- [8] [Android 9 behavior changes - cleartext network traffic](https://developer.android.com/about/versions/pie/android-9.0-changes-28)
- [9] [Android Open Source Project - ART just-in-time compiler and code cache](https://source.android.com/docs/core/runtime/jit-compiler)
