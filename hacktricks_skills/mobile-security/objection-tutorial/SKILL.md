---
name: hacktricks-objection-tutorial
description: >-
  Hacktricks playbook: Objection Tutorial. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: Objection Tutorial
...
[**Objection**](https://github.com/sensepost/objection) is a runtime mobile exploration toolkit, powered by [Frida](https://www.frida.re). It was built with the aim of helping assess mobile applications and their security posture without the need for a jailbroken or rooted mobile device.<sup>[[1]](#references)</sup>
...
If you are using a **non-rooted device**, a common workflow is to patch the APK to embed **Frida Gadget**, reinstall it, and then connect to `Gadget`:<sup>[[3]](#references)</sup>
...
If the application still crashes or exits as soon as Objection/Frida attaches, continue with [this anti-instrumentation workflow](../android-anti-instrumentation-and-ssl-pinning-bypass.md).
...
![Basic Actions - Environment](<../../../images/image (220).png>)
...
![Environment - Frida Information](<../../../images/image (1093).png>)
...
If the app runs root checks, SSL pinning or anti-tampering **during startup**, run the hook before the REPL finishes collecting environment data:<sup>[[2]](#references)</sup>
...
![Static analysis made Dynamic - List activities, receivers and services: android hooking list activities](<../../../images/image (1016).png>)
...
![List activities, receivers and services - Getting current activity: android hooking get current activity](<../../../images/image (813).png>)
...
![Getting current activity - Search Classes: android hooking search classes asvid.github.io.fridaapp](<../../../images/image (507).png>)
...
![Search Classes - Search Methods of a class: android hooking search methods asvid.github.io.fridaapp MainActivity](<../../../images/image (929).png>)
...
![Search Methods of a class - List declared Methods of a class with their parameters: android hooking list class methods asvid.github.io.fridaapp.MainActivity](<../../../images/image (299).png>)
...
The application's [source code](https://github.com/asvid/FridaApp/blob/master/app/src/main/java/asvid/github/io/fridaapp/MainActivity.kt) shows that `MainActivity.sum()` runs every second. Dump its arguments, return value, and backtrace each time it is called:

```bash
android hooking watch class_method asvid.github.io.fridaapp.MainActivity.sum --dump-args --dump-backtrace --dump-return
```

![Hooking being easy - Hooking (watching) a method: android hooking watch class method asvid.github.io.fridaapp.MainActivity.sum --dump-args --dump-backtrace --dump-return](<../../../images/image (1086).png>)
...
![Hooking (watching) a method - Hooking (watching) an entire class: If you play with the application while the class is hooked you will see when each function is being called , its...](<../../../images/image (861).png>)
...
![Hooking (watching) an entire class - Changing boolean return value of a function: From the source code you can see that the function checkPin gets a String as argument and returns a...](<../../../images/image (883).png>)

After the hook is active, any value entered in the PIN field is accepted:

![The hooked checkPin method accepts any value entered in the PIN field](<../../../images/image (228).png>)
...
![Changing boolean return value of a function - Class instances: android heap print instances](<../../../images/image (1095).png>)
...
![Dump - List: memory list modules](<../../../images/image (286).png>)

At the bottom of the list, you can see Frida:

![Frida shown in the loaded-module list](<../../../images/image (1097).png>)
...
![Listing exports from a loaded module](<../../../images/image (298).png>)
...
- There is no shortcut comparable to `sslpinning` for hooking every common cryptographic API and displaying ciphertext, plaintext, keys, IVs, and algorithms.

## References

- [1] [Objection (SensePost)](https://github.com/sensepost/objection)
- [2] [Objection wiki: Early Instrumentation](https://github.com/sensepost/objection/wiki/Early-Instrumentation)
- [3] [Objection wiki: Patching Android Applications](https://github.com/sensepost/objection/wiki/Patching-Android-Applications)
