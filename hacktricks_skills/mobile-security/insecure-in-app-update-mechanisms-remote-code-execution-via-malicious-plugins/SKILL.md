---
name: hacktricks-insecure-in-app-update-mechanisms-remote-code-execution-via-malicious-plugins
description: >-
  Hacktricks playbook: Insecure In-App Update Mechanisms – Remote Code Execution via Malicious Plugins. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: Insecure In-App Update Mechanisms – Remote Code Execution via Malicious Plugins
...
```
The response body is an XML document whose `<FileData>` nodes contain Base64-encoded, DES-ECB encrypted JSON describing each available plugin.<sup>[[1]](#references)</sup>
...
`addon.py` example that silently swaps the original metadata:<sup>[[1]](#references)</sup>
