---
name: hacktricks-ios-basics
description: >-
  Hacktricks playbook: iOS Basics. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: iOS Basics
...
- `/Applications`: Contains the installed native system applications on the device (for example `/Applications/Calculator.app`).
- `/private/var/containers/Bundle/Application/<UUID>`: Contains the application bundles for installed apps. In practice you will also see the shortened `/var/containers/...` path in logs and tools.
- `/private/var/mobile/Containers/Data/Application/<UUID>`: Contains the private data container for each installed app.
- `/private/var/mobile/Containers/Shared/AppGroup/<UUID>`: Shared container used by apps, widgets, and extensions that belong to the same **App Group**. Shared `UserDefaults`, SQLite DBs, caches, exported files, and token hand-off artifacts often end up here.
- `/private/var/mobile/Containers/Data/PluginKitPlugin/<UUID>`: Per-extension / PluginKit data. This is especially interesting when testing share extensions, widgets, document pickers, or temporary imported media.
...
- `/var/jb`: On modern **rootless jailbreaks**, this usually points to the writable jailbreak bootstrap under `/private/preboot/<hash>/...`; many modern tweaks, binaries, and jailbreak indicators live here instead of directly under `/`.<sup>[[5]](#references)</sup>
...
When extracting data, copy the **three files together**. Recent rows often live only in the WAL file, so pulling just `<name>.db` can produce an incomplete or misleading snapshot.<sup>[[1]](#references)</sup>
...
Applications are installed in a specific directory (`private/var/mobile/Applications/{random ID}`) and have restricted read access to certain system areas and functionalities, such as SMS and phone calls. Access to protected areas triggers a pop-up request for user permission.<sup>[[3]](#references)[[6]](#references)</sup>
...
During a review, inspect both the **private app container** and any **App Group shared container**. Teams often protect the main login database correctly but leave shared `UserDefaults`, exported documents, caches, or helper SQLite files with a weaker protection class.

A quick runtime check from inside the app is:

```swift
let attrs = try FileManager.default.attributesOfItem(atPath: path)
print(attrs[.protectionKey] ?? "no protection")
```
...
**Management and access** to the Keychain data are handled by the **`securityd` daemon**, based on specific app entitlements like `Keychain-access-groups` and `application-identifier`.<sup>[[1]](#references)</sup>

From a tester's perspective, **shared Keychain access groups widen the trust boundary**: every app, extension, widget, or companion target that is entitled for the same group can potentially read or write those entries. Review the whole app suite, not just the main bundle.

#### **Keychain API Operations**

The Keychain API, detailed at [Apple's Keychain Services documentation](https://developer.apple.com/library/content/documentation/Security/Conceptual/keychainServConcepts/02concepts/concepts.html), provides essential functions for secure storage management:<sup>[[1]](#references)</sup>
...
Brute-forcing the Keychain password involves either attacking the encrypted key directly or attempting to guess the passcode on the device itself, hindered significantly by secure enclave's enforcement of a delay between failed attempts.<sup>[[1]](#references)</sup>
...
### Quick triage from an IPA

A fast first pass against an IPA usually answers most of the "what should I inspect next?" questions:

```bash
unzip -q app.ipa -d /tmp/app
APP=$(find /tmp/app/Payload -maxdepth 1 -name '*.app' | head -n 1)
plutil -p "$APP/Info.plist" | rg 'UsageDescription|NSAppTransportSecurity|CFBundleURLTypes|LSApplicationQueriesSchemes|UIRequiredDeviceCapabilities'
codesign -d --entitlements :- "$APP" 2>/dev/null
test -f "$APP/embedded.mobileprovision" && security cms -D -i "$APP/embedded.mobileprovision" | plutil -p -
find "$APP/PlugIns" -maxdepth 2 \( -name '*.appex' -o -name Info.plist \) 2>/dev/null
```

This is a great place to spot:

- **ATS exceptions** via `NSAppTransportSecurity`
- **Cross-app interaction pivots** via `CFBundleURLTypes` or `LSApplicationQueriesSchemes`
- **Extra attack surface** in extensions under `PlugIns/`
- **Interesting entitlements** such as `get-task-allow`, `com.apple.security.application-groups`, `keychain-access-groups`, or `com.apple.developer.associated-domains`

Those keys are excellent pivots into [custom URL schemes](ios-custom-uri-handlers-deeplinks-custom-schemes.md), [Universal Links](ios-universal-links.md), and [app extensions](ios-app-extensions.md).
...
- **`get-task-allow`**: Development/debug build; especially interesting if you are assessing a test build or a re-signed IPA.
- **`com.apple.security.application-groups`**: Shared file container across app/extension boundaries.
- **`keychain-access-groups`**: Cross-target Keychain sharing.
- **`com.apple.developer.associated-domains`**: Universal Links, web credentials, and related domain trust.
- **Networking / VPN / extension entitlements**: Often indicate privileged OS integration or a larger attack surface than the main UI suggests.

If the entitlements file isn't directly present in the package, extract it from the Mach-O as explained in [Extracting Entitlements from Compiled Application](extracting-entitlements-from-compiled-application.md).
...
- [3] [iOS Platform APIs - OWASP MASTG](https://mas.owasp.org/MASTG/0x06h-Testing-Platform-Interaction/)
