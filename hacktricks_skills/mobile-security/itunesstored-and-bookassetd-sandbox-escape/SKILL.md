---
name: hacktricks-itunesstored-and-bookassetd-sandbox-escape
description: >-
  Hacktricks playbook: `itunesstored` and `bookassetd` Sandbox Escape. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: `itunesstored` and `bookassetd` Sandbox Escape
...
4. Knowledge of the **Books system-group UUID** so the Stage 1 write lands in the right container (found via syslog).<sup>[[1]](#references)</sup>
...
`bookassetd` respects the EPUB ZIP format: `mimetype` must be the first uncompressed entry. To map EPUB contents to the MobileGestalt cache, build a directory tree that mirrors the desired path relative to `ZPLISTPATH`.<sup>[[1]](#references)</sup>
...
- **Public PoC** – <https://github.com/hanakim3945/bl_sbx> ships baseline SQLite/EPUB templates you can customize.<sup>[[2]](#references)</sup>
...
- [1] [itunesstored & bookassetd sbx escape](https://hanakim3945.github.io/posts/download28_sbx_escape/)
- [2] [bl_sbx PoC repository](https://github.com/hanakim3945/bl_sbx)
