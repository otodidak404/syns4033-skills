---
name: hacktricks-spoofing-llmnr-nbt-ns-mdnsdns-and-wpad-and-relay-attacks
description: >-
  Hacktricks playbook: Spoofing LLMNR, NBT-NS, mDNS/DNS and WPAD and Relay Attacks. Exploitation techniques, enumeration, and attack patterns.
...
- Responder can automate WPAD attacks, directing clients to malicious WPAD servers.<sup>[[3]](#references)</sup>
...
Each tool can be configured to operate through a SOCKS proxy if necessary, enabling attacks even with indirect network access.<sup>[[2]](#references)</sup>

### MultiRelay Operation

MultiRelay is executed from the _**/usr/share/responder/tools**_ directory, targeting specific IPs or users.<sup>[[4]](#references)</sup>
...
- Default ports: 8530/tcp HTTP, 8531/tcp HTTPS.<sup>[[6]](#references)[[7]](#references)</sup>

Reconnaissance
- Unauthenticated
  - Scan for listeners: nmap -sSVC -Pn --open -p 8530,8531 -iL <hosts>
  - Sniff HTTP WSUS traffic via L2 MITM and log active clients/endpoints with wsusniff.py (HTTP only unless you can make clients trust your TLS cert).<sup>[[10]](#references)</sup>
- Authenticated
  - Parse SYSVOL GPOs for WSUS keys with MANSPIDER + regpol (wsuspider.sh wrapper summarises WUServer/WUStatusServer/UseWUServer).<sup>[[11]](#references)</sup>
...
3) Start ntlmrelayx with the HTTP listener (requires Impacket support for HTTP listener; see PRs below):<sup>[[8]](#references)[[9]](#references)</sup>
...
- Passive interception of WSUS over HTTPS is ineffective unless clients trust your certificate. Without a trusted cert or other TLS break, the NTLM handshake can’t be harvested/relayed from WSUS HTTPS traffic.<sup>[[6]](#references)[[7]](#references)</sup>

Notes
- WSUS was announced deprecated but remains widely deployed; HTTP (8530) is still common in many environments.<sup>[[13]](#references)</sup>
- Useful helpers: wsusniff.py (observe HTTP WSUS check-ins), wsuspider.sh (enumerate WUServer/WUStatusServer from GPOs), NetExec reg-query at scale.<sup>[[10]](#references)[[11]](#references)</sup>
- Impacket restored HTTP listener support for ntlmrelayx in PR #2034 (originally added in PR #913).<sup>[[8]](#references)[[9]](#references)</sup>

### Metasploit 6.5: HTTP → SMB/LDAP relay and NTLMRelay2Self

If you want to keep the whole relay workflow inside **Metasploit**, version **6.5** added two dedicated HTTP relay servers plus an automated local privilege-escalation chain:<sup>[[21]](#references)</sup>

- `auxiliary/server/relay/http_to_smb`: runs an HTTP listener, requests NTLM auth, relays it to the configured `RHOSTS` over SMB (`RPORT` defaults to `445`) and opens an **interactive SMB session**.<sup>[[22]](#references)</sup>
- `auxiliary/server/relay/http_to_ldap`: same idea for LDAP (`RPORT` defaults to `389`) and opens an **interactive LDAP session**. Current module logic only relays **NTLMv1** to LDAP because NTLMv2 MIC breaks the relay, so the target must still be willing to emit NTLMv1.<sup>[[23]](#references)</sup>
...
`exploit/windows/local/ntlm_relay_2_self` automates a **relay-to-self** LPE from a foothold on a **domain-joined workstation**. Based on the current module code, the practical prerequisites are:<sup>[[24]](#references)</sup>
...
Unlike NTLM relay, the hop is limited to the *same host* but, if you target a protocol that lets you write to LDAP, you can chain into **Resource-Based Constrained Delegation (RBCD)** or **AD CS enrollment** and pop **NT AUTHORITY\SYSTEM** in a single shot.<sup>[[19]](#references)[[20]](#references)</sup>

For detailed info about this attack check:

- [https://googleprojectzero.blogspot.com/2021/10/using-kerberos-for-authentication-relay.html](https://googleprojectzero.blogspot.com/2021/10/using-kerberos-for-authentication-relay.html)
- [https://decoder.cloud/2025/04/24/from-ntlm-relay-to-kerberos-relay-everything-you-need-to-know/](https://decoder.cloud/2025/04/24/from-ntlm-relay-to-kerberos-relay-everything-you-need-to-know/).<sup>[[20]](#references)</sup>
...
1. **Shared key:** source and target SPNs belong to the same computer account (default on Windows servers).<sup>[[25]](#references)</sup>
...
Synacktiv documented a **new Kerberos coercion/relay primitive** that bypassed the first SMB reflection mitigations by abusing **inconsistent Unicode normalization** across Windows DNS, Kerberos/SPN lookup, and SMB ticket acceptance.<sup>[[16]](#references)</sup>
...
- `DnsCache` self-name checks use `CompareStringW` with only **`NORM_IGNORECASE`**.<sup>[[18]](#references)</sup>
- SPN lookups in AD ultimately depend on **ESE/NTDS search keys** derived from `LCMapStringEx(..., 0x31403)` (`LCMAP_SORTKEY`, `NORM_IGNORECASE`, `NORM_IGNOREKANATYPE`, `NORM_IGNORENONSPACE`, `NORM_IGNOREWIDTH`, `SORT_STRINGSORT`).<sup>[[17]](#references)</sup>
- Therefore, two strings can be **different for client-side self-comparison** but still **collide during SPN lookup** on the DC.
- SMB then accepts the relayed AP-REQ as long as the service ticket decrypts under the same machine account key and the local-auth checks are otherwise satisfied.<sup>[[16]](#references)</sup>

Practical constraints and workflow:

1. A pure hostname variant tends to fail because **LDAP/DNS uniqueness checks** can hit the same normalized collision and reject the record as already existing.<sup>[[16]](#references)</sup>
...
4. Some relay tooling may need a **small patch** to stop enforcing strict ASCII/hostname equality on the relayed target name because the ticket `sname` can contain Unicode.<sup>[[16]](#references)</sup>

Representative chain:<sup>[[16]](#references)</sup>
...
- [1] [HTB: Breach – Writable SMB share lures + Responder capture → NetNTLMv2 crack](https://0xdf.gitlab.io/2026/02/10/htb-breach.html)
- [2] [SMB Relay Attack Tutorial (Intrinium)](https://intrinium.com/smb-relay-attack-tutorial/)
- [3] [LLMNR and NBT-NS Poisoning Using Responder (4ARMED)](https://www.4armed.com/blog/llmnr-nbtns-poisoning-using-responder/)
- [4] [Pwning with Responder - A Pentester's Guide (NotSoSecure)](https://www.notsosecure.com/pwning-with-responder-a-pentesters-guide/)
- [5] [Practical guide to NTLM Relaying in 2017 (A.K.A getting a foothold in under 5 minutes)](https://byt3bl33d3r.github.io/practical-guide-to-ntlm-relaying-in-2017-aka-getting-a-foothold-in-under-5-minutes.html)
- [6] [WSUS Is SUS: NTLM Relay Attacks in Plain Sight (TrustedSec)](https://trustedsec.com/blog/wsus-is-sus-ntlm-relay-attacks-in-plain-sight)
- [7] [GoSecure – Abusing WSUS to enable NTLM relaying attacks](https://gosecure.ai/blog/2021/11/22/gosecure-investigates-abusing-windows-server-update-services-wsus-to-enable-ntlm-relaying-attacks)
- [8] [Impacket PR #2034 – Restore HTTP server in ntlmrelayx](https://github.com/fortra/impacket/pull/2034)
- [9] [Impacket PR #913 – HTTP relay support](https://github.com/fortra/impacket/pull/913)
- [10] [WSUScripts – wsusniff.py](https://github.com/Coontzy1/WSUScripts/blob/main/wsusniff.py)
- [11] [WSUScripts – wsuspider.sh](https://github.com/Coontzy1/WSUScripts/blob/main/wsuspider.sh)
- [12] [MS-WSUSOD – Windows Server Update Services: Server-to-Client Protocol](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-wsusod/e00a5e81-c600-40d9-96b5-9cab78364416)
- [13] [Microsoft – WSUS deprecation announcement](https://techcommunity.microsoft.com/blog/windows-itpro-blog/windows-server-update-services-wsus-deprecation/4250436)
- [14] [RelayKing v1.0](https://github.com/depthsecurity/RelayKing-Depth)
- [15] [Depth Security – Introducing RelayKing: Relay to Royalty](https://www.depthsecurity.com/blog/introducing-relayking-relay-to-royalty/)
- [16] [Synacktiv - Bypassing Windows authentication reflection mitigations for SYSTEM shells - Part 2](https://www.synacktiv.com/en/publications/bypassing-windows-authentication-reflection-mitigations-for-system-shells-part.html)
...
- [19] [Using Kerberos for Authentication Relay Attacks (Project Zero)](https://projectzero.google/2021/10/using-kerberos-for-authentication-relay.html)
- [20] [From NTLM relay to Kerberos relay: Everything you need to know (Decoder)](https://decoder.cloud/2025/04/24/from-ntlm-relay-to-kerberos-relay-everything-you-need-to-know/)
- [21] [Rapid7 - Metasploit Framework 6.5 Released](https://www.rapid7.com/blog/post/pt-metasploit-framework-6-5-released)
- [22] [Metasploit source - `http_to_smb`](https://github.com/rapid7/metasploit-framework/blob/master/modules/auxiliary/server/relay/http_to_smb.rb)
- [23] [Metasploit source - `http_to_ldap`](https://github.com/rapid7/metasploit-framework/blob/master/modules/auxiliary/server/relay/http_to_ldap.rb)
- [24] [Metasploit source - `ntlm_relay_2_self`](https://github.com/rapid7/metasploit-framework/blob/master/modules/exploits/windows/local/ntlm_relay_2_self.rb)
- [25] [Semperis - Exploiting Ghost SPNs and Kerberos Reflection for SMB Server Privilege Elevation](https://www.semperis.com/blog/exploiting-ghost-spns-and-kerberos-reflection-for-smb-server-privilege-elevation/)
