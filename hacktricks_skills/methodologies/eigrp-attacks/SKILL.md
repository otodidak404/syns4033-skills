---
name: hacktricks-eigrp-attacks
description: >-
  Hacktricks playbook: EIGRP Attacks. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: EIGRP Attacks
...
**The first four attack patterns were originally described by** [**in9uz's “Cisco Nightmare: Pentesting Cisco Networks Like a Devil”**](https://medium.com/@in9uz/cisco-nightmare-pentesting-cisco-networks-like-a-devil-f4032eb437b9).<sup>[[7]](#references)</sup> The original Medium post is currently unavailable.
...
Before you try to inject routes, capture a legitimate HELLO / UPDATE exchange and extract the fields that define the local adjacency.<sup>[[1]](#references)</sup>
...
Nmap's `broadcast-eigrp-discovery` works by sending a HELLO to `224.0.0.10` and parsing the returned UPDATE packets, which is useful to enumerate prefixes before attempting a more intrusive route injection.<sup>[[3]](#references)</sup>
...
Minimal default-route example from the Scapy script.<sup>[[5]](#references)</sup>
...
Scapy ships an EIGRP contrib layer with TLVs like `EIGRPParam` and `EIGRPIntRoute`, which is enough to craft UPDATEs for route injection. The following example is adapted from the `davidbombal/scapy` EIGRP route injection script.<sup>[[5]](#references)[[8]](#references)</sup>
...
The same repo includes quick "fake neighbor" scripts that sniff a real EIGRP packet and replay it with a spoofed source IP to create phantom neighbors (useful for CPU/neighbor-table pressure).<sup>[[10]](#references)</sup>

Scapy also exposes primitives that are useful when you need higher-fidelity emulation instead of a single UPDATE.<sup>[[8]](#references)</sup>
...
That matters because reliable transport and observed STUB behavior affect whether a fake neighbor survives long enough to learn routes or receive queries.<sup>[[1]](#references)</sup>

## **EIGRP for IPv6**

EIGRP for IPv6 is a separate address-family transported over IPv6. It still uses EIGRP packet format / TLVs, but it multicasts to `FF02::A`. From an offensive perspective, that means a dual-stack segment may expose an EIGRP attack surface even when the IPv4 side looks clean.<sup>[[1]](#references)</sup>
...
Minimal Scapy example for IPv6 route injection.<sup>[[8]](#references)</sup>
...
If the IPv6 next hop inside the route TLV is zeroed, receivers fall back to the IPv6 source address in the packet header. That makes source spoofing and correct link-local addressing especially important during EIGRPv6 testing.<sup>[[1]](#references)[[8]](#references)</sup>
...
- [1] [RFC 7868 - Cisco's Enhanced Interior Gateway Routing Protocol (EIGRP)](https://www.rfc-editor.org/rfc/rfc7868.html)
- [2] [Configuring EIGRP SHA-256 Authentication - Cisco IOS IP Routing: EIGRP Configuration Guide](https://www.cisco.com/c/en/us/td/docs/ios-xml/ios/iproute_eigrp/configuration/15-mt/ire-15-mt-book/ire-sha-256.html)
- [3] [broadcast-eigrp-discovery NSE Script - Nmap](https://nmap.org/nsedoc/scripts/broadcast-eigrp-discovery.html)
- [4] [Routopsy: hacking routing with routers - SensePost](https://sensepost.com/blog/2020/routopsy-hacking-routing-with-routers/)
- [5] [eigrp-route-injection.py - davidbombal/scapy](https://github.com/davidbombal/scapy/blob/main/eigrp-route-injection.py)
- [6] [EIGRP Graceful Shutdown - NetworkLessons](https://networklessons.com/eigrp/eigrp-graceful-shutdown)
- [7] [Cisco Nightmare: Pentesting Cisco Networks Like a Devil](https://medium.com/@in9uz/cisco-nightmare-pentesting-cisco-networks-like-a-devil-f4032eb437b9)
- [8] [scapy.contrib.eigrp - Scapy Documentation](https://scapy.readthedocs.io/en/latest/api/scapy.contrib.eigrp.html)
- [9] [eigrp - Nmap NSE Library Documentation](https://nmap.org/nsedoc/lib/eigrp.html)
- [10] [eigrp-many-fake-neighbors.py - davidbombal/scapy](https://github.com/davidbombal/scapy/blob/main/eigrp-many-fake-neighbors.py)
