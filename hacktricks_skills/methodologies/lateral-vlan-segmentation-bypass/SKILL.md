---
name: hacktricks-lateral-vlan-segmentation-bypass
description: >-
  Hacktricks playbook: Lateral VLAN Segmentation Bypass. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: Lateral VLAN Segmentation Bypass
...
If direct access to a switch is available, VLAN segmentation can be bypassed by reconfiguring the connected port as a trunk, creating virtual interfaces for target VLANs, and assigning addresses dynamically (DHCP) or statically.<sup>[[1]](#references)</sup>
...
On a Cisco port left in a DTP-negotiating mode, a host can send DTP frames that cause trunk negotiation and expose the VLANs allowed on the resulting trunk.  A static access port, optionally with `switchport nonegotiate`, prevents this; the exact result depends on both port modes.<sup>[[3]](#references)[[7]](#references)</sup>

*Yersinia* can send DTP packets and enable trunking, while the `dtp-spoof` project provides a PoC for testing this configuration:<sup>[[9]](#references)[[10]](#references)</sup>
...
The following helper passively fingerprints the port’s DTP state:<sup>[[11]](#references)</sup>
...
*VoIP Hopper* is available as a Kali package and supports CDP, DHCP options **176/242**, and LLDP-MED discovery/spoofing:<sup>[[4]](#references)[[14]](#references)</sup>
...
Whether this works depends on CDP/LLDP-MED exposure, voice-VLAN policy, DHCP behavior, and port-security controls.<sup>[[4]](#references)</sup>
