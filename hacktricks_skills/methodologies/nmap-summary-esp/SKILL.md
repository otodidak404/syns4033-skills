---
name: hacktricks-nmap-summary-esp
description: >-
  Hacktricks playbook: Nmap Summary (ESP). Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: Nmap Summary (ESP)
...
- **`-n`**: No DNS
- **`-R`**: DNS always
- **`--system-dns`**: Force the OS resolver instead of Nmap's parallel stub resolver. Useful when `/etc/hosts`, split-DNS, or resolver plugins return data that Nmap's direct queries do not. It is slower, and since Nmap 7.96 forward lookups are already parallelized, this is usually only needed for resolver compatibility.<sup>[[6]](#references)</sup>
- **`--dns-servers <server[,server],...>`**: Force specific DNS servers for reverse lookups. Useful in internal assessments to query authoritative or internal resolvers directly, or to bounce `-sL`/reverse-DNS traffic away from the tester's default resolvers.
...
**--version-light** Alias of `--version-intensity 2`. Very useful for a first pass against large ranges or slow UDP services.

**--version-all** Alias of `--version-intensity 9`. Forces all probes and is useful when a service only answers rare probes.

**--allports** Forces version detection on ports excluded by `nmap-service-probes` (notably TCP/9100). Be careful: on some printers or raw socket listeners this can make them print the probe data.
...
If you wish to see the options in packets sent and received, specify --packet-trace. For more information and examples of using IP options with Nmap, see [http://seclists.org/nmap-dev/2006/q3/52](http://seclists.org/nmap-dev/2006/q3/52).<sup>[[9]](#references)</sup>
...
**-oG file** Greppable output. It still works, but it is deprecated; XML is the better format for automation because new Nmap features are added there first. Keep using `-oN` if you want `--resume`, and prefer `-oX`/`-oA` for machine parsing.<sup>[[7]](#references)</sup>

**-oA file** All except -oS

**--webxml** Changes the XML stylesheet reference to `https://nmap.org/svn/docs/nmap.xsl`, making the XML easier to open as HTML on another machine.<sup>[[7]](#references)</sup>

**--stylesheet <path|url>** Use a custom XSL stylesheet. `--webxml` is just a shortcut to the hosted official stylesheet.
...
**--resume file** Resumes an interrupted scan from a normal (`-oN`) or grepable (`-oG`) output file. In current workflows it is common to keep `-oN` for resumability and `-oX` for parsing.<sup>[[7]](#references)</sup>
...
- Since Nmap 7.94, UDP port scan (`-sU`) and version detection (`-sV`) use the same `nmap-service-probes` payload source. A UDP response from the scan phase can immediately feed version matching, so `-sU -sV --version-light` is now a good first pass against large or lossy ranges.<sup>[[6]](#references)</sup>
- Since Nmap 7.94, `-sV` can also probe UDP services hidden behind DTLS, which is useful for modern management/ICS gear that wraps UDP protocols in DTLS.<sup>[[6]](#references)</sup>
- Nmap 7.95 added a large batch of new service fingerprints, including `grpc`, `mysqlx`, `remotemouse`, and `tuya`, plus new ICS-focused NSE coverage such as `hartip-info` and `iec61850-mms`. If you are scanning OT or embedded estates, updating Nmap matters more than adding custom probes too early.<sup>[[6]](#references)</sup>
- Since Nmap 7.96, forward DNS lookups are parallelized too. Large hostname lists are much faster now, so `--system-dns` should usually be reserved for compatibility issues instead of performance.<sup>[[6]](#references)</sup>

## Speed Up Nmap Service scan x16

According [**to this post**](https://joshua.hu/nmap-speedup-service-scanning-16x) you can speed up the nmap service analysis by modifying all the **`totalwaitms`** values in **`/usr/share/nmap/nmap-service-probes`** to **300** and **`tcpwrappedms`** to **200**.<sup>[[8]](#references)</sup>
...
In hardened or minimal Linux environments (containers, appliances), dynamically linked Nmap binaries often fail due to missing runtime loaders or shared libraries (e.g., /lib64/ld-linux-x86-64.so.2, libc.so). Building your own statically linked Nmap and bundling NSE data allows execution without installing system packages.<sup>[[1]](#references)</sup>

High-level approach
- Use a clean amd64 Ubuntu builder via Docker.
- Build OpenSSL and PCRE2 as static libraries.<sup>[[3]](#references)[[4]](#references)</sup>
- Build Nmap linking statically and using the included libpcap/libdnet to avoid dynamic deps.<sup>[[5]](#references)</sup>
...
- Deterministic builds reduce supply-chain risk vs downloading opaque “static” binaries.<sup>[[2]](#references)</sup>
...
- [1] [Compiling static Nmap binary for jobs in restricted environments](https://www.pentestpartners.com/security-blog/compiling-static-nmap-binary-for-jobs-in-restricted-environments/)
- [2] [Static Nmap Binary Generator (helper tool)](https://github.com/0x5ubt13/static_nmap_binary_generator)
- [3] [OpenSSL sources](https://www.openssl.org/source/)
- [4] [PCRE2 releases](https://github.com/PCRE2Project/pcre2/releases)
- [5] [Nmap source tarballs](https://nmap.org/dist/)
- [6] [Nmap Change Log](https://nmap.org/changelog.html)
- [7] [Nmap Output Formats](https://nmap.org/book/man-output.html)
- [8] [Speeding up Nmap service scanning 16x](https://joshua.hu/nmap-speedup-service-scanning-16x)
- [9] [seclists.org - Q3 - 52](http://seclists.org/nmap-dev/2006/q3/52)
