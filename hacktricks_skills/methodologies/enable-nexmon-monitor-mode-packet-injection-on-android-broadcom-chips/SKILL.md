---
name: hacktricks-enable-nexmon-monitor-mode-packet-injection-on-android-broadcom-chips
description: >-
  Hacktricks playbook: Enable NexMon Monitor Mode & Packet Injection on Android (Broadcom chips). Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: Enable NexMon Monitor Mode & Packet Injection on Android (Broadcom chips)
...
* Optional Kali NetHunter chroot to run classic wireless tools (aircrack-ng, wifite, mdk4 …) directly against the internal interface.<sup>[[6]](#references)</sup>
...
* NexMon Magisk ZIP or self-compiled patch providing:
  ...
  * `/system/xbin/nexutil`
* Hijacker ≥ 1.7 (arm/arm64) – [https://github.com/chrisk44/Hijacker](https://github.com/chrisk44/Hijacker).<sup>[[3]](#references)</sup>
...
* On `bcm4375b1`, the upstream NexMon tree currently ships monitor/injection patches for **18.38.18** (Galaxy S10) and **18.41.8.9** (Galaxy S20). If your ROM has already moved to a newer firmware such as **18.41.113** or **18.41.117**, reusing the old patch often leads to a misleading state where `nexutil` appears to succeed but `iw wlan0 info` still shows `managed` mode or capture tools see **zero frames**.<sup>[[5]](#references)[[7]](#references)</sup>
* Upstream now also contains raw `bcm4375b1` firmware directories for **18.41.113** and **18.41.117**, but the public **monitor/injection patch** directories still stop at **18.38.18** and **18.41.8.9**. Having the newer blob in `firmwares/` does **not** mean there is already a matching Android monitor/injection patch in `patches/`.<sup>[[5]](#references)[[7]](#references)</sup>
* On the older S10/S20 workflow, `getenforce` usually needs to be **Permissive** (or the SELinux policy must be patched) or `nexutil` / `libnexmon.so` ioctls may be blocked before they reach the Broadcom driver.<sup>[[4]](#references)</sup>
* If you only want to validate the patch quickly, upstream NexMon also supports the minimal monitor-mode toggle `nexutil -m2`; there is no need to create a separate `wlan0mon` interface.<sup>[[2]](#references)</sup>
...
Hijacker can toggle monitor mode automatically before running `airodump`, `wifite`, etc.  In **Settings -> Advanced** add the following entries (edit the library path if your module differs):<sup>[[6]](#references)</sup>
...
The Kali Galaxy S10 guide uses `nexutil -s0x613 -i -v2` as its monitor-mode setup command.<sup>[[6]](#references)</sup> In the official parser, `-s` selects a custom set-command/ioctl, `-i` marks the custom-command value as an integer, and `-v2` supplies the value `2`.<sup>[[8]](#references)</sup>

The parser does not define the firmware-specific meaning of custom command `0x613`; do not describe it as `FCAP_FRAME_INJECTION` without verifying the exact S10 patch source.<sup>[[8]](#references)</sup>

* **`-s0x613`**   Select custom set-command/ioctl number `0x613` (the `0x` prefix is parsed as hexadecimal).
* **`-i`**         Interpret the custom-command value as an integer.
* **`-v2`**        Supply integer value `2` to the selected custom command.
* **`-m2`**        Set monitor mode through nexutil's dedicated monitor-mode option; this upstream flow is separate from the S10 guide's custom command.<sup>[[2]](#references)</sup>
* **`-m0`**        Set monitor mode value `0` to return to managed mode.<sup>[[2]](#references)</sup>

After running *Enable monitor mode* you should see the interface in monitor state and be able to capture raw frames with:<sup>[[6]](#references)</sup>
```bash
...
```bash
# S10 guide monitor-mode setup command
svc wifi disable && sleep 2 && ifconfig wlan0 up && nexutil -s0x613 -i -v2
...
For passive monitor mode, prefer the upstream-documented `nexutil -m2` flow; do not assume that omitting the S10-specific `-s0x613` custom command has the same effect on every patched firmware.<sup>[[2]](#references)</sup>
...
Stock user-space tools in Kali do not know about NexMon, but you can force them to use it via `LD_PRELOAD`.<sup>[[6]](#references)</sup>
...
With patched firmware and the appropriate preload helper, user-space tools can use the internal interface in place of an external adapter.<sup>[[2]](#references)[[6]](#references)</sup>

## When the ROM is newer than the published patch
If `nexutil -V` reports a newer `bcm4375b1` firmware than the patch you flashed, the problem is usually **firmware-porting**, not Hijacker itself.<sup>[[5]](#references)[[7]](#references)</sup>
...
* `nexutil -m2` or the S10 guide command returns success but `iw wlan0 info` still says `managed` – this often means a **firmware/driver mismatch** (for example, flashing an `18.41.8.9` patch onto a phone now running `18.41.113+`).
...
* SELinux blocking library – set device to *Permissive* or fix module context: `chcon u:object_r:system_lib_file:s0 libnexmon.so`.<sup>[[4]](#references)</sup>

---

## References

- [1] [Hijacker on the Samsung Galaxy S10 with wireless injection](https://forums.kali.org/t/hijacker-on-the-samsung-galaxy-s10-with-wireless-injection/10305)
- [2] [NexMon – firmware patching framework](https://github.com/seemoo-lab/nexmon)
- [3] [Hijacker (aircrack-ng GUI for Android)](https://github.com/chrisk44/Hijacker)
- [4] [SELinux needs to be disabled to make nexutil work on new devices](https://github.com/seemoo-lab/nexmon/issues/471)
- [5] [Samsung Galaxy S20's version 18.41.113 seems partially completed in the repository?](https://github.com/seemoo-lab/nexmon/discussions/629)
- [6] [Installing NetHunter on the Samsung Galaxy S10](https://www.kali.org/docs/nethunter/installing-nethunter-on-the-samsung-galaxy-s10/)
- [7] [Galaxy S10 bcm4375b1 patch update](https://github.com/seemoo-lab/nexmon/issues/631)
- [8] [Nexutil command-line parser](https://raw.githubusercontent.com/seemoo-lab/nexmon/master/utilities/nexutil/nexutil.c)
