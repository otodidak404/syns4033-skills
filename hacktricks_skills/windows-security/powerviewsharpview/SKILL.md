---
name: hacktricks-powerviewsharpview
description: >-
  Hacktricks playbook: PowerView/SharpView. Exploitation techniques, enumeration, and attack patterns.
source: https://book.hacktricks.xyz/
tags: [pentest, exploit, hacking]
---

# Hacktricks: PowerView/SharpView
...
The final PowerView version published in the now-archived PowerSploit project is available in its development branch: [PowerView.ps1](https://github.com/PowerShellMafia/PowerSploit/blob/dev/Recon/PowerView.ps1).<sup>[[1]](#references)</sup>

[**SharpView**](https://github.com/tevora-threat/SharpView) is a .NET port of PowerView. The examples below are a practical command reference; confirm parameters against the version you are running because PowerView aliases and behavior vary between releases.<sup>[[1]](#references)[[2]](#references)</sup>
...
Get-NetComputer -Unconstrained | select samaccountname #DCs always appear but aren't useful for privilege escalation
...
#Asks the DC for all computers and checks each one for local admin access (very noisy). RPC and SMB ports must be open.
...
Get-NetDomainController -Domain mydomain.local #Get domain controllers for the specified domain
...
Get-NetGroup -Domain mydomain.local #Get groups from a specific domain
Get-NetGroup 'Domain Admins' #Get all data of a group
Get-NetGroup -AdminCount | select name,memberof,admincount,member | fl #Search administrative groups
...
# Computers
Get-DomainComputer -Properties DnsHostName #Get the DNS hostnames of domain computers
## Get-DomainComputer is kind of the same as Get-NetComputer
Get-NetComputer #Get all computer objects
Get-NetComputer -Ping #Send a ping to check if the computers are working
Get-NetComputer -Unconstrained #DCs always appear but aren't useful for privilege escalation
...
If an attacker has **high privileges over a GPO**, they may be able to **escalate privileges** by granting permissions to a user, adding a local administrator to a host, or creating an immediate scheduled task.\
...
#Find interesting ACEs (modification rights held by unexpected objects with RID > 1000)
Find-InterestingDomainAcl -ResolveGUIDs

#Check whether any interesting permission is related to a username/group
...
```bash
Get-NetFileServer #Search file servers, where many users may have active sessions
Find-DomainShare -CheckShareAccess #Search readable shares
...
Get-DomainForeignUser #Get users with privileges in other domains inside the forest
...
# If -CheckAccess is used, it also checks for local administrator access on the hosts.
...
```bash
#This is not a PowerView command; it comes from Microsoft's Active Directory PowerShell module
...
```bash
# Use alternate credentials with any supported function
...
- [1] [PowerSploit - PowerView.ps1](https://github.com/PowerShellMafia/PowerSploit/blob/dev/Recon/PowerView.ps1)
- [2] [SharpView - .NET port of PowerView](https://github.com/tevora-threat/SharpView)
